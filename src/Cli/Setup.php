...
namespace My_Awesome_Plugin\Cli;
...
class Setup {
	...
	public function __invoke( array $args, array $assoc_args ): void {
		...
		// Replace in files
		if (
			! $this->replaceInFiles( 'my-awesome-plugin', $this->slug, [ '.*\.php', '.*\.js', '.*\.json' ] )
			|| ! $this->replaceInFiles( 'my_awesome_plugin', str_replace( '-', '_', $this->slug ), [ '.*\.php' ] )
			|| ! $this->replaceInFiles( 'My_Awesome_Plugin', $this->namespace, [ '.*\.php', '.*\.json' ] )
			|| ! $this->replaceInFiles( 'MY_AWESOME_PLUGIN', strtoupper( $this->namespace ), [ '.*\.php', '.*\.json' ] )
			|| ! $this->replaceInFiles( 'My Awesome Plugin', $this->name, [ '.*\.php', '.*README\.txt' ] )
		) {
	...
	private function rename_files(): void {
		if (
			! rename( $this->path . '/src/My_Awesome_Plugin.php', $this->path . "/src/$this->namespace.php" )
			|| ! rename( $this->path . '/my-awesome-plugin.php', $this->path . "/$this->slug.php" )
		) {
